...
public class MediaDigestDb {
    ...
    public static List<MediaDigest> getMediaDigest(String custId, StoreUpType type) throws Exception {
        String selectString="select id,author,media_id,media_chapter_id,media_name,bar_id,first_catetory_id,first_catetory_name,content,type*1 as type,column_id,column_name,stars,review_cnt,collect_cnt,share_cnt,\t\n" +
                ...
                "is_show,is_del,sign_ids,day_or_night,mood,weight,operator,last_update_date,sort_page,is_paper_book from media_digest where type in ("+type.getDigestType()+") and author like \"%"+custId+"%\" and is_del=0 ORDER BY create_date desc";
        List<MediaDigest> medias = DbUtil.selectList(com.dangdang.config.Config.YCDBConfig, selectString, MediaDigest.class);
        return medias;
    }
}
