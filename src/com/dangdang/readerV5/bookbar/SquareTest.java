			...
			for(int i=0; i< bModuleList.size(); i++){
				...
				if(bModuleList.get(i).getType().toString().equals("1")){
					...
					for(int j=0; j<bCotentList.size(); j++){
						sql = "select * from bar where bar_id ="+bCotentList.get(j).getContentId();
				...
				if(bModuleList.get(i).getType().toString().equals("3")){
					...
					for(int k=0; k<tCotentList.size(); k++){					
						...
						tContentOfDB.setBeginDate(tCotentList.get(k).getBeginDate().toString());
						tContentOfDB.setEndDate(tCotentList.get(k).getEndDate().toString());
