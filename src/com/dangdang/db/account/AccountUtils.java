...
public class AccountUtils {
    ...
    public  static List<AttachAccountItems> getAttachLingDangDetail(String custid,TestDevice device) throws Exception {
        return DbUtil.selectList(Config.ACCOUNTDBConfig,"select * from attach_account_items where cust_id="+custid+" and (`status`=1 or (`status`!=1 and creation_date>=DATE_SUB(NOW(),INTERVAL 10 DAY))) and consume_source='"+device+"'  order by effective_date,attach_account_items_id DESC ",AttachAccountItems.class);
    }
