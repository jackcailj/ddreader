...
public class PageLayoutDetail extends FixtureBase{
    ...
    protected void dataVerify() throws Exception {
        if(reponseResult.getStatus().getCode()==0){
            //获取页面布局
            List<MediaLayoutDetail> layoutDetails = MediaLayoutDetailDb.getLayoutDetail(paramMap.get("layoutCode"));
            if(layoutDetails.size()>0) {
                ...
                dataVerifyManager.add(new ListVerify(reponseResult.getData().getList(), new ArrayList(), true));
            }
        }
        super.dataVerify();
    }
